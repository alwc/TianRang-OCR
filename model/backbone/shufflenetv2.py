...
class InvertedResidual(nn.Module):
    def __init__(self, inp, oup, stride):
        ...
        if not (1 <= self.max_stide <= 3):
            raise ValueError('illegal stride value')

        branch_features = oup // 2
        assert (self.max_stide != 1) or (inp == branch_features << 1)
        ...
        self.branch2 = nn.Sequential(
            nn.Conv2d(inp if (self.max_stide  > 1) else branch_features,
...
class ShuffleNetV2(nn.Module):
    def __init__(self, stages_repeats, stages_out_channels, algorithm, **kwargs):
        ...
        self.output_channel = stages_out_channels[-2]
        ...
        for name, repeats, output_channels in zip(
            ...
            if algorithm == 'rec':
                seq = [InvertedResidual(input_channels, output_channels, (2, 1))]
            elif algorithm == 'det':
