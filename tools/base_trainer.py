...
class BaseTrainer:
    def __init__(self, config, model, criterion):
        ...
        if 'transforms' in self.config['dataset']['train']['dataset']['args'].keys():
    ...
    def _laod_checkpoint(self, checkpoint_path, resume):
        ...
        :param checkpoint_path: Checkoptimizerpoint path to be resumed
        """
        self.logger_info("Loading checkpoint: {} ...".format(checkpoint_path))
        checkpoint = torch.load(checkpoint_path, map_location=torch.device('cpu'))
        missing_keys, unexpected_keys = self.model.load_state_dict(checkpoint['state_dict'], strict=resume)
