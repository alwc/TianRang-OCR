...
class LPR():
    ...
    def decode_ctc(self,y_pred):
        ...
        for i,one in enumerate(res):
            if one<len(chars) and (i==0 or (one!=res[i-1])):
                results+= chars[one]
                confidence.append(table_pred[i][one])
        # confidence/= len(results)
        return results, min(confidence)
    ...
    def predict(self, src):
        t0 = time.time()
        text, confidence = self.segmentation_free_recognition(src)
        return text, confidence.astype(float), time.time() - t0
