...
class CropWordBox():
    ...
    def __init__(self, value):
        ...
        :param value: 如果是tuple，从中随机从选择一个值；如果是list，随机选择一个list[0]<=value<=list[1]的值;如果是int或者float，直接使用该值。
                      正值表示外扩，负值表示收缩
        '''
        self.value = value
    ...
    def _get_value(self):
        if isinstance(self.value, list):
            ...
            num = random.random()
            value = self.value[0] + (self.value[1] -self.value[0])*num
    ...
    def __call__(self, data):
        ...
        for i, v in enumerate(texts):
            ...
            word_img = self.crop_image_by_bbox(im.copy(), text_polys[i], value)
            word_imgs.append(word_img)
        data['word_imgs'] = word_imgs
        return data

    @staticmethod
    def crop_image_by_bbox(image, box, scale):
        '''
        暂时不考虑竖排文字
        :param image:
        :param box: 4x2 左上角开始，顺时针四个点
        :param scale: 抠图周围外扩或收缩的比例
        :return:
        '''
        ...
        new_w = int(w*scale)
        new_h = int(h*scale)
        new_box = np.array([[new_w-w, new_h-h], [new_w, new_h-h], [new_w, new_h], [new_w-w, new_h]]).astype(np.float32)

        M = cv2.getPerspectiveTransform(np.float32(box), new_box)
        warped = cv2.warpPerspective(image, M, (int(2*new_w - w), int(2*new_h - h)))
        return warped
